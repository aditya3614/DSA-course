          ...
          {!showResult ? (
            <div>
              ...
              <div className="text-black w-full mt-8 flex items-center justify-center">
                {checked ? (
                  <button
                    className="hover:bg-green-300 w-1/4 rounded bg-green-200"
                    onClick={nextQuestion}
                  >
                    {activeQuestion == Questions.length - 1 ? "Finish" : "Next"}
                  </button>
                ) : (
                  <button className="bg-white rounded w-1/4" disabled>
                    {activeQuestion == Questions.length - 1 ? "Finish" : "Next"}
                  </button>
                )}
              </div>
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              {(result.score / 25) * 100 < 50 ? (
                <div className="flex flex-col items-center">
                  <p>
                    Your score is below 50% so you can not move forward you have
                    to relearn Day {currentDay} topics and you can also take
                    help from our AI chatbot in understanding efficiently
                  </p>
                  <Link href={`/course/day-${currentDay}`}>
                    <button className={buttonVariants()}>
                      Go back to day{currentDay} Learning
                    </button>
                  </Link>
                </div>
              ) : (
                <div className="flex flex-col gap-4 items-center">
                  ...
                  <h2 className="font-semibold">
                    Congrats you have scored more than 50% in your hence you are
                    eligible to move forward, you day {currentDay + 1} Course is
                    unlocked now!!
                  </h2>
